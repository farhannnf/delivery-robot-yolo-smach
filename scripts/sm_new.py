...
class PositionDatabase:
    ...
    def get_person_position(self, name):
        ...
        Ini berbeda dari get_position() yang mengembalikan posisi ROBOT.
        Digunakan untuk menghitung error posisi robot terhadap person
        sesuai arahan Pak Yani (21 April 2026).

        Return: (person_x, person_y) atau None jika tidak ada.
        """
        if self.database is None:
            return None
        for entry in self.database:
...
class StateIdle(smach.State):
    ...
    def execute(self, userdata):
        ...
        # Ambil posisi person dari database (untuk perhitungan error)
        ...
        if person_pos:
            ...
            rospy.logwarn(">>> person_x/person_y tidak tersedia di database")
        ...
        if self.client.wait_for_result(rospy.Duration(timeout_duration)):
            ...
            if state == 3:  # SUCCEEDED
                ...
                try:
                    ...
                    # ERROR 1: Posisi robot vs goal (posisi robot saat scanning)
                    # Mengukur seberapa akurat navigasi move_base
                    error_goal = math.sqrt(
                        (actual_x - userdata.goal_x)**2 +
                        (actual_y - userdata.goal_y)**2
                    ) * 100.0
                    rospy.loginfo(">>> Error posisi ROBOT vs GOAL: %.2f cm",
                                  error_goal)

                    # ERROR 2: Posisi robot vs person (posisi orang di database)
                    # Mengukur seberapa dekat robot sampai ke posisi person
                    # Arahan Pak Yani (21 April 2026): "error posisi robot
                    # terhadap person perlu"
                    if (hasattr(userdata, 'person_x') and
                        ...
                        # Ringkasan kedua error
                        rospy.loginfo("=" * 50)
                        rospy.loginfo("  RINGKASAN ERROR POSISI:")
                        rospy.loginfo("  Robot aktual    : (%.4f, %.4f)",
                                      actual_x, actual_y)
                        rospy.loginfo("  Goal (robot DB) : (%.4f, %.4f)",
                                      userdata.goal_x, userdata.goal_y)
                        rospy.loginfo("  Person (DB)     : (%.4f, %.4f)",
                                      userdata.person_x, userdata.person_y)
                        rospy.loginfo("  Error vs Goal   : %.2f cm",
                                      error_goal)
                        rospy.loginfo("  Error vs Person : %.2f cm",
                                      error_person)
                        rospy.loginfo("=" * 50)
                    else:
                        rospy.logwarn(">>> person_x/person_y tidak tersedia "
...
class StateComplete(smach.State):
    ...
    def execute(self, userdata):
        ...
        # Origin tidak punya person_x/person_y
        userdata.person_x = None
        userdata.person_y = None
